...
import { FaCrosshairs, FaList, FaDirections, FaSearch, FaBolt, FaSpinner } from 'react-icons/fa';
...
const MapMoveListener = ({ onLocationChange, debounceDelay = 1500, isProgrammaticMove }) => {
  const timeoutRef = useRef(null);
  const lastCenterRef = useRef(null);
  const programmaticMoveRef = useRef(false);

  // Update programmatic move ref when prop changes
  useEffect(() => {
    programmaticMoveRef.current = isProgrammaticMove;
  }, [isProgrammaticMove]);
  ...
  const map = useMapEvents({
    moveend: () => {
      ...
      if (programmaticMoveRef.current) {
        console.log('Skipping API call - programmatic move');
        return;
      }
...
const MapSection = () => {
  ...
  const [isProgrammaticMove, setIsProgrammaticMove] = useState(false); // Add this
...
useEffect(() => {
  ...
  // Handle map location change (only for user moves)
  const handleMapLocationChange = useCallback((location) => {
    ...
    // Clear selected station when user moves map
...
// Center on user location
  ...
  const handleMarkerClick = useCallback((station, index) => {
    ...
    if (coords.lat && coords.lng) {
      // Set programmatic move flag before flying
      setIsProgrammaticMove(true);
